...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _batch_save_episode_video(self, start_episode: int, end_episode: int | None = None) -> None:
        ...
        # Episode metadata is buffered while recording. Always flush and
        # reload it before batch encoding. During resumed recording,
        # ``self.meta.episodes`` is non-None but only contains the episodes
        # that existed when the dataset was opened; checking for None alone
        # therefore leaves newly saved episodes out of bounds.
        self.meta._close_writer()
        self.meta.episodes = load_episodes(self.root)

        # Episode metadata and frame data are chunked independently. Use the
        # metadata file coordinates here; using data/file_index breaks resumed
        # datasets as soon as their frame data rolls over to another file.
        ...
        for ep_idx in range(start_episode, end_episode):
            ...
            # Let the next episode in this deferred batch append after the
            # video coordinates that were just created, rather than using the
            # stale pre-batch latest_episode entry.
            ...
            episode_df.to_parquet(episode_df_path)
            self.meta.episodes = load_episodes(self.root)
            # _save_episode_video uses list-valued latest_episode metadata to
            # append the next episode to the current video file.
            current_ep = self.meta.episodes[ep_idx]
            self.meta.latest_episode = {key: [value] for key, value in current_ep.items()}
    ...
    def _save_episode_video(
        ...
        latest_chunk = _scalar_index(latest_ep.get(chunk_key)) if latest_ep else None
        latest_file = _scalar_index(latest_ep.get(file_key)) if latest_ep else None
        if episode_index == 0 or latest_chunk is None or latest_file is None:
            # Initialize indices for a new dataset made of the first episode data
            chunk_idx, file_idx = 0, 0
            previous_video = None
            if self.meta.episodes is not None:
                # Newly saved batched episodes have video columns filled with
                # nulls. Find the most recent actually encoded episode.
                for prev_idx in range(min(episode_index, len(self.meta.episodes)) - 1, -1, -1):
                    candidate = self.meta.episodes[prev_idx]
                    old_chunk_idx = _scalar_index(candidate.get(chunk_key))
                    old_file_idx = _scalar_index(candidate.get(file_key))
                    if old_chunk_idx is not None and old_file_idx is not None:
                        previous_video = (old_chunk_idx, old_file_idx)
                        break
            if previous_video is not None:
                old_chunk_idx, old_file_idx = previous_video
            ...
            chunk_idx = latest_chunk
            file_idx = latest_file
    ...
    def clear_episode_buffer(self, delete_images: bool = True) -> None:
        ...
        if delete_images:
            ...
            # Video observations are also staged as image frames until the
            # episode is encoded. They must be removed when Q discards a
            # pending episode or Escape discards an active one.
            for cam_key in [*self.meta.image_keys, *self.meta.video_keys]:
