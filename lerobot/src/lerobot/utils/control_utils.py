    ...
    def on_release(key):
        ...
        try:
            ...
            if key == keyboard.Key.right:
                ...
                if events["input_locked"]:
                    print("Space ignored while the previous episode is being committed. Please wait...")
                else:
                    print("Space released. Toggling manual recording...")
                    events["toggle_recording"] = True
                    events["exit_early"] = True
                    last_event_time = now
            elif isinstance(key, keyboard.KeyCode) and key.char and key.char.lower() == "q":
                if events["input_locked"]:
                    print("Q ignored while the previous episode is being committed. Please wait...")
                elif events["is_recording"]:
                    print("Q ignored while recording. Press Space to finish the episode, then press Q.")
                else:
                    print("Q pressed. Deleting the most recently completed episode...")
