...
std::map<std::pair<int, int>, mb_dm_handle*> g_persistent_handles;
...
extern "C" int mb_dm_open(const char* library_path, int device_type, int device_index,
                          uint8_t selected_channel,
                          uint32_t can_baudrate, uint32_t canfd_baudrate, mb_dm_handle** out,
                          char* err_buf, size_t err_len)
{
    if (!library_path || !out || device_index < 0) {
...

    {
        ...
        auto it = g_persistent_handles.find({device_type, device_index});
        if (it != g_persistent_handles.end() && it->second && it->second->dev) {
            auto* h = it->second;
    ...
    if (device_index >= count) {
        set_err(err_buf, err_len, "DM_Device index " + std::to_string(device_index) +
            " out of range; found " + std::to_string(count) + " matching device(s)");
        delete h;
        return -1;
    }

    if (!h->api.device_get(h->ctx, &h->dev, device_index) || !h->dev) {
        set_err(err_buf, err_len, "dmcan_device_get(index=" +
    ...
    {
        std::lock_guard<std::mutex> persistent_lock(g_persistent_mutex);
        g_persistent_handles[{device_type, device_index}] = h;
    }
